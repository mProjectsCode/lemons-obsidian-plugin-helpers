...
// packages/repo-automation/src/release.ts
...
// packages/repo-automation/src/utils/shellUtils.ts
function exec(c, cwd) {
  return Bun.spawn(parseArgsStringToArgv(c), { cwd });
}
async function $(cmd, cwd, verboseness = 1 /* NORMAL */) {
  if (verboseness === 1 /* NORMAL */ || verboseness === 2 /* VERBOSE */) {
    if (cwd !== undefined) {
      console.log(`
...
async function $seq(cmds, onError, onSuccess, cwd, verboseness = 1 /* NORMAL */) {
  const results = [];
...
// packages/repo-automation/src/utils/utils.ts
...
class ParserContext {
  ...
  getAndClearLatestError() {
    const error = this.latestError;
    this.latestError = undefined;
    return error;
  }
  mergeLatestError(other) {
    if (other === undefined) {
      return;
    }
...
function createParsingErrorMessage(str, parseFailure, verbose) {
...
class ParsingError extends Error {
  constructor(str, parseFailure) {
    super(createParsingErrorMessage(str, parseFailure, true));
  }
}
function mapIndexToLineColumn(str, index) {
  let line = 1;
  let column = 1;
...
class P {
  static sequence(...parsers) {
    ...
    return new Parser(function _sequence(context) {
      const value = new Array(parsers.length);
  ...
  static range(begin, end) {
    ...
    return P_HELPERS.testCharCode(function _range(charCode) {
...
class Parser {
  ...
  marker() {
    return P.sequenceMap(function _marker(from, value, to) {
      return {
        value,
        range: { from, to }
      };
  ...
  namedMarker(name) {
    return P.sequenceMap(function _namedMarker(from, value, to) {
      return {
        value,
        name,
        range: { from, to }
  ...
  node(fn) {
    return P.sequenceMap(function _node(from, value, to) {
...
class ParserHelpers {
  ...
  testCharCode(fn, failureMessage) {
    return new Parser(function _test(context) {
  ...
  testCharCodes(fn, failureMessage) {
    return new Parser(function _test(context) {
      let i = context.position;
      for (;i < context.input.length; i++) {
...
var charCodeA = 97;
var charCodeZ = 122;
var charCodeAUpper = 65;
var charCodeZUpper = 90;
...
`) {
  ...
  static binaryRightRange(operatorsParser, operandParser, combine) {
    return P.sequenceMap((others, last, to) => {
      return others.reverse().reduce((acc, y) => {
        const [from, operand, operator] = y;
        return combine({ from, to }, operand, operator, acc);
      }, last);
    }, P.sequence(P_UTILS.position(), operandParser, operatorsParser.trim(this.optionalWhitespace())).many(), operandParser, P_UTILS.position());
  }
  static binaryLeftRange(operatorsParser, operandParser, combine) {
    return P.sequenceMap((from, first, others) => {
      return others.reduce((acc, y) => {
        const [operator, operand, to] = y;
        return combine({ from, to }, acc, operator, operand);
      }, first);
    }, P_UTILS.position(), operandParser, P.sequence(operatorsParser.trim(this.optionalWhitespace()), operandParser, P_UTILS.position()).many());
  }
  static func(name, args, combine) {
    const nameParser = typeof name === "string" ? P.string(name) : name;
    return P.sequenceMap((name2, _lBracket, _1, args2, _2, _rBracket) => {
      return combine(name2, args2);
    }, nameParser, P.string("("), this.optionalWhitespace(), args, this.optionalWhitespace(), P.string(")"));
  }
}

// packages/repo-automation/src/utils/versionUtils.ts
...
class CanaryVersion extends Version {
  canary;
...
var numberParser = P_UTILS.digits().map((x) => Number.parseInt(x)).chain((x) => {
  if (Number.isNaN(x)) {
...
function getIncrementOptions(version) {
  const moment = import_moment.default();
...
function getRepoPath(repoRoot, fileName) {
  return path2.join(repoRoot, fileName);
}
