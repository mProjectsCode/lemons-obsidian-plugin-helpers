...
export var Verboseness;
(function (Verboseness) {
    Verboseness[Verboseness["QUITET"] = 0] = "QUITET";
    Verboseness[Verboseness["NORMAL"] = 1] = "NORMAL";
    Verboseness[Verboseness["VERBOSE"] = 2] = "VERBOSE";
})(Verboseness || (Verboseness = {}));
function exec(c, cwd) {
    return Bun.spawn(stringArgv(c), { cwd: cwd });
}
export async function $(cmd, cwd, verboseness = Verboseness.NORMAL) {
...
export async function $seq(cmds, onError, onSuccess, cwd, verboseness = Verboseness.NORMAL) {
    const results = [];
...
export async function $choice(message, options) {
    console.log(`${message} `);
...
export async function $confirm(message, onReject) {
    while (true) {
